...
│       ├── behaviors.py       # Core behaviors/logic
...
- **Version Bumps**: update `pyproject.toml`, `CHANGELOG.md` and update the constants in `src/security_dependency_pinning/__init__conf__.py` according to `pyproject.toml`
    - Automation rewrites `src/security_dependency_pinning/__init__conf__.py` from `pyproject.toml`, so runtime code imports generated constants instead of querying `importlib.metadata`.
...
This library uses a simple two-layer architecture:
- **cli.py**: CLI interface using rich-click
- **behaviors.py**: Core business logic and behaviors
